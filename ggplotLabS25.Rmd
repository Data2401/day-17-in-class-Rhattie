...
```{r}
heights %>% ggplot(aes(x=height)) + geom_histogram(bins = 64, fill = "#800020"
)
...
```{r}
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth  = 1, fill = "#800020"
)
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth  = 5, fill = "#800020")

heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth  = 10, fill = "#800020")

heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth  = 20, fill = "#800020")


#1 works and the others make it bigger  and 20 is near unusable
```
...
```{r}
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth  = 1, fill = "#800020"
) + geom_density(color = "blue", linewidth = 3) + 
  theme_minimal() 
```
...
```{r}
heights %>% ggplot(aes(x=height, group= sex, fill = sex)) +geom_density(color= "#d3cabb" , linewidth = 1)+ theme_clean()
```
...
```{r}
heights %>% ggplot(aes(x=height, group= sex, fill = sex)) +geom_density(color= "#d3cabb" , linewidth = 1)+ theme_bw()
...
```{r}
heights %>% ggplot(aes(x=height, group= sex, fill = sex)) +geom_density(color= "#C4673E" , linewidth = 1, alpha= .5)+ theme_bw()
...
```{r}
heights %>% ggplot(aes(x=height, group= sex, fill = sex))+geom_boxplot(color= "#C4673E" , linewidth = 1, alpha= .5)+ theme_bw()
```
...
```{r}
ggplot(votedata %>% filter(year == 2000)) + geom_col(aes(x = state, y = demVote)) + ylab("Democratic Vote")
...
ggplot(votedata %>% filter(year == 2000)) +geom_col(aes(x = state, y = demVote)) + coord_flip()  +ylab("Democratic Vote")
...
```{r}
ggplot(votedata %>% filter(year == 2000)) + geom_col(aes(x = reorder(state, demVote), y = demVote)) + coord_flip()+ylab("Democratic Vote") + xlab("State")
...
ggplot(votedata %>% filter(year == 2008 | year == 2016)) + 
  geom_col(aes(x = reorder(state, demVote), y = demVote, fill = south)) + 
  geom_hline(yintercept = 50, linetype = "dashed", color = "#621B21") + 
  coord_flip() + 
  facet_wrap(~ year) + 
  labs(title = "Percent Dem Vote by State, in 2000",
       subtitle = "Colored by State location", 
...
votedata %>%
  ...
  ggplot(aes(x = year, y = avg_dem_vote, color = south, group = south)) +
